...
async function migrate0823Corrected() {
    ...
    try {
        ...
        console.log('=== Step 2: Registering all 31 Newcomers (Col 3 = 새참자, Col 4 = 인도자) ===');
        const newcomerItems = data.filter(x => x.type === 'newcomer');
        const newcomerMap = {}; // row -> NC_id

        for (const nc of newcomerItems) {
            // Check if newcomer's name itself is already registered in addressbook
            ...
            console.log(`[ID ${insertId}] [${nc.area_code}구역] 새참자: ${nc.name} | 인도자: ${nc.guide_name} (성도등재: ${isReg ? matchedCode : '새참자유지'})`);
        }
